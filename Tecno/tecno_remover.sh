#!/system/bin/sh
# Tecno Bloatware Remover for Shizuku
# Remove unwanted pre-installed apps from Tecno devices
...
print_header() {
    echo "=================================================="
    print_colored $BLUE "Tecno Bloatware Remover for Shizuku"
...
create_backup() {
    ...
    BACKUP_FILE="/sdcard/tecno_backup_$(date +%Y%m%d_%H%M%S).txt"
...
# Tecno bloatware packages categorized by risk level
SAFE_PACKAGES="
...
batch_removal() {
    ...
    echo ""
    print_colored $YELLOW "This will remove all SAFE Tecno bloatware packages."
...
show_menu() {
    echo ""
    print_colored $BLUE "=== TECNO BLOATWARE REMOVER MENU ==="
    ...
    case $choice in
        ...
        4)
            ...
            ;;
        5)
            print_colored $GREEN "Exiting Tecno Bloatware Remover."
            exit 0
            ;;
        *)
