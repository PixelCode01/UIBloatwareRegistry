#!/system/bin/sh
# Nothing Bloatware Remover for Shizuku
# Remove unwanted pre-installed apps from Nothing devices
...
print_header() {
    echo "=================================================="
    print_colored $BLUE "Nothing Bloatware Remover for Shizuku"
...
create_backup() {
    ...
    BACKUP_FILE="/sdcard/nothing_backup_$(date +%Y%m%d_%H%M%S).txt"
...
# Nothing bloatware packages categorized by risk level
SAFE_PACKAGES="
com.nothing.camera:Nothing Camera
com.nothing.gallery:Nothing Gallery
...
batch_removal() {
    ...
    echo ""
    print_colored $YELLOW "This will remove all SAFE Nothing bloatware packages."
...
show_menu() {
    echo ""
    print_colored $BLUE "=== NOTHING BLOATWARE REMOVER MENU ==="
    ...
    case $choice in
        ...
        4)
            ...
            ;;
        5)
            print_colored $GREEN "Exiting Nothing Bloatware Remover."
            exit 0
            ;;
        *)
